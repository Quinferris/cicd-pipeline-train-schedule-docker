pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("qferris/train-schedule")
                    app.inside {
    ...
    steps {
        ...
        withCredentials ([usernamePassword(credentialsId: 'webserverlogin', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
            script {
                sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@${env.prod_ip} \"docker pull qferris/train-schedule:${env.BUILD_NUMBER}\""
                try {
